...
$comments = [];
foreach ($all_comments_as_arrays as $newcom) {
    $newvalue = new Comment;
    $newvalue ->  fillFromArray($newcom);
    $comments []= $newvalue;
}
...
<body>
    ...
    <div class="comments">
        ...
        <textarea  name="text" id="" cols="30" rows="10"><?=htmlspecialchars($comment ->text)?></textarea>
        <input type="submit" value="Submit">
        
        </form>
        <?php foreach($comments as $newvalue): ?>
        <p> <?= htmlspecialchars($newvalue->author) ?> </p>
        <?php endforeach; ?>
    </div>
